...
$("#addBot").onclick = () => {
  ...
  ioClient.emit("bot:add", { id, name, auth });
...
ioClient.on("bot:list", list => {
  ...
  cont.querySelectorAll("[data-del]").forEach(btn => btn.onclick = () => ioClient.emit("bot:remove", btn.dataset.del));
...
function handleMove(dir, on) {
  if (!currentBotId) return;
  const continuous = $("#mvContinuous").checked;
  if (continuous) {
    mvHeld[dir] = on;
