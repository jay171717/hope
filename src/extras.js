...
/**
 * Robust sneak toggle + auto-sleep + anti-afk cleanup helpers.
 */
  ...
  entry._sleepTimer = setInterval(async () => {
    try {
      ...
      try {
        ...
        io.emit("bot:log", { id: entry.id, line: `Auto-sleep: failed sleep - ${err?.message || err}` });
        try {
...
// ---------- Anti-AFK cleanup ----------
export function clearAntiAfk(entry) {
  if (entry?._antiAfk) {
    entry._antiAfk.stop();
    entry._antiAfk = null;
  }
