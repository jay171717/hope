...
import { startAntiAfk, stopAntiAfk } from "./antiAfk.js"; // ✅ NEW
...
export class BotManager {
  ...
  _applyTweaksAfterSpawn(e) {
    ...
    if (e.tweaks.antiAfk) startAntiAfk(e, this.io); // ✅ NEW
  }
  ...
  setTweaks(id, toggles) {
    ...
    if (b) {
      ...
      if (toggles.antiAfk !== undefined) { // ✅ NEW
  ...
  _ensureFollow(e) {
    ...
    e._followTimer = setInterval(() => {
      try {
        ...
        if (plEntry && plEntry.entity) {
          try { b.pathfinder.setGoal(new goals.GoalFollow(plEntry.entity, 1), true); } catch {}
        }
      } catch {}
    }, 2000);
  }
  ...
  _clearTimersAndListeners(e) {
    ...
    if (e._sneakInterval) clearInterval(e._sneakInterval);
